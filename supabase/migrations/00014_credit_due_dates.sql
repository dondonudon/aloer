-- Migration: Credit due dates.
-- Adds optional due_date columns to sales and purchase_orders. Required for
-- new credit transactions; existing rows stay NULL and are excluded from
...
CREATE INDEX IF NOT EXISTS idx_purchase_orders_due_date ON purchase_orders (due_date) WHERE due_date IS NOT NULL;

-- ============================================================
-- FUNCTION: create_sale_transaction (rebuilt with due_date support)
-- Identical to 00011 except for: v_due_date parsing, validation when
-- isCreditSale, and persistence into the sales row.
-- ============================================================
...
BEGIN
  ...
  v_is_credit := COALESCE((sale_payload->>'isCreditSale')::boolean, false);
  v_due_date := NULLIF(sale_payload->>'dueDate', '')::date;
  ...
  IF v_is_credit AND (sale_payload->>'resellerId') IS NULL THEN
    RAISE EXCEPTION 'A reseller must be selected for credit sales';
  END IF;

  IF v_is_credit AND v_due_date IS NULL THEN
    RAISE EXCEPTION 'A due date is required for credit sales';
  END IF;
  ...
  INSERT INTO sales (
    ...
    payment_method, reseller_id, status, created_by, created_at, idempotency_key, due_date
  )
  VALUES (
    ...
    'completed', auth.uid(), now(), v_idempotency_key, v_due_date
  );
