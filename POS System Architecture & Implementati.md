...
> **Last updated:** May 12, 2026 (In-app notification inbox)
...
| **`get_sales_summary` RPC** | Migration `00015_get_sales_summary.sql` moves sales aggregation from app-level JS to a Postgres function. Accepts `p_start_date`, `p_end_date` (timestamptz), `p_timezone` (IANA name, default `UTC`), and `p_payment_type` (optional filter). Returns one row per calendar day (timezone-aware) with `total_transactions`, `total_revenue`, `total_cogs`, `gross_profit`, `total_discount`, `total_delivery_fee`. Sales report and dashboard use this. Payment type filter is also surfaced as a UI filter on the sales list (`/sales`) and sales report (`/reports`) pages. |
| **In-app notification inbox** | Migration `00016_notifications.sql` adds a `notifications` table (`user_id`, `title`, `body`, `url`, `tag`, `is_read`, `created_at`). `UNIQUE (user_id, tag)` deduplicates tagged reminders so cron re-runs don't create duplicate inbox entries. `sendPushToUser()` in `push-server.ts` now upserts a `notifications` row before dispatching the device push — users without a push subscription still accumulate inbox entries. `/notifications` page lists the 100 most recent entries. Per-notification checkbox marks a single item read (optimistic UI + `markNotificationRead` server action); a "Mark all as read" button fires `markAllNotificationsRead`. Bell icon added to sidebar footer (same row as language/theme buttons) shows a red unread dot when there are unread notifications and navigates to `/notifications`. Unread count is fetched in `AuthenticatedLayout` and passed to the sidebar at render time. |
...
| **Migrations squashed** | 16 iterative migration files — `00001_schema.sql` is the base; `00002` – `00016` extend it incrementally. `00015` adds the `get_sales_summary` RPC; `00016` adds the `notifications` table. |
